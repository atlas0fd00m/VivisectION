...
class DirecedGraphCanvas(vq_memory.VivCanvasBase):
    ...
    ########################## SPLIT THIS BETTER..  Make the GRAPH object responsible for rendering the graph
    ### REQUIREMENTS FOR DirectedGraphs:
    ### * Must maintain a "curva" for each graph node?  Even if it's unique but nonsense
    ...
    def contextMenuEvent(self, event):
        if self._canv_curva is not None:
            #### HOW TO HANDLE THIS BETTER?  set buildContextMenu in each View?  In the Canvas?  Based on NodeID?  EdgeID?
...
def reset():
    DirectedGraphView.viewidx = itertools.count()

class DirectedGraphView(vq_hotkey.HotKeyMixin, e_qt_memory.EnviNavMixin, QWidget, vq_save.SaveableWidget, viv_base.VivEventCore):
    ...
    def __init__(self, vw, vwqgui):
        ...
        self.setEnviNavName('Graph%d' % next(self.viewidx))
        ...
        self.mem_canvas = DirectedGraphCanvas(vw, syms=vw, parent=self)
    ...
    def rendToolsSetName(self, user=None):
        ...
        mwname, ok = QInputDialog.getText(self, 'Set Graph View Name', 'Name', text=curname)
        if ok:
    ...
    ######### Expressly getting Code XRefs (by _canv_curva)
    ...
    #########################  extricate and make _renderNode
